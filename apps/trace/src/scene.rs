...
pub struct Transform(pub Transform3);

#[derive(Debug)]
pub enum Geometry {
    TriangleMesh { positions: Vec<Vec3>, indices: Vec<UVec3> },
    Quad { transform: Transform3, size: Vec2 },
}
...
pub fn create_cornell_box_scene(variant: &CornellBoxVariant) -> Scene {
    ...
    let camera_transform = scene.add_transform(Transform(
        Isometry3::new(Vec3::new(0.278, 0.273, -0.8), Rotor3::identity()).into_transform(),
    ));
    ...
    if matches!(variant, CornellBoxVariant::Instances) {
        let extra_transforms: Vec<_> = (1..10)
            .map(|i| {
                let f = i as f32;
                scene.add_transform(Transform(
                    Isometry3::new(Vec3::new(-0.01 * f, 0.02 * f, 0.0), Rotor3::from_rotation_xz(0.05 * f))
                        .into_transform(),
                ))
            })
            .collect();
